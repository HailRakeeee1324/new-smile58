...
const BRANCHES = [
  { name: 'Новая улыбка — Светлая 11', phone: '+79022077008', street: 'ул. Светлая, 11' },
  { name: 'Новая улыбка — Радужная 10', phone: '+79272888306', street: 'ул. Радужная, 10' },
  { name: 'Новая улыбка — Антонова 76', phone: '+79521969335', street: 'ул. Антонова, 76' },
];
...
function clinicJson() {
  return {
    ...
    department: BRANCHES.map((branch) => ({
      ...
      address: { '@type': 'PostalAddress', streetAddress: branch.street, addressLocality: 'Пенза', addressCountry: 'RU' },
    })),
...
const privacyStaticHtml = `<main class="seo-static legal-static">
  <section><h1>Политика обработки персональных данных</h1><p>Редакция от 18.06.2026. Документ применяется к сайту new-smile58.ru, форме записи, сервисам аналитики и защиты от спама.</p></section>
  <section><h2>Операторы</h2><p>ООО «Новая улыбка», ОГРН 1215800003088, ИНН 5829006081, адрес: 440514, Пензенская область, Пензенский район, с. Засечное, ул. Светлая, д. 11, помещ. 112.</p><p>ООО «АЭЛИТА», ОГРН 1235800004615, ИНН 5835142646, адрес: 440502, Пензенская область, Пензенский район, с. Алферьевка, Садовый проезд, д. 13.</p></section>
  <section><h2>Какие данные обрабатываются</h2><p>Имя, телефон, выбранный район, страница и время отправки заявки, источник перехода, UTM-метки, cookies, IP-адрес, сведения о браузере и устройстве, а также сведения о факте согласия.</p><p>Сайт не предназначен для передачи диагнозов, медицинских документов, паспортных данных и банковских реквизитов.</p></section>
  <section><h2>Цели</h2><ul><li>обработка заявки и обратный звонок;</li><li>подбор филиала и времени приёма;</li><li>защита формы от спама;</li><li>аналитика посещаемости;</li><li>подтверждение законности обработки.</li></ul></section>
  <section><h2>Сроки</h2><p>Данные заявки обрабатываются до достижения цели, но не более 12 месяцев после последнего взаимодействия, если закон не требует иного. Сведения о согласии могут храниться в пределах применимых сроков исковой давности.</p></section>
  <section><h2>Сервисы</h2><p>Для работы сайта могут использоваться Яндекс Метрика, Yandex SmartCaptcha, хостинг-провайдер и Telegram для передачи заявки ответственным администраторам.</p></section>
  <section><h2>Права пользователя</h2><p>Пользователь вправе запросить сведения об обработке, потребовать уточнения, блокирования или уничтожения данных и отозвать согласие. Для предварительной связи: <a href="${PHONE_LINK}">${PHONE}</a>.</p></section>
</main>`;

const consentStaticHtml = `<main class="seo-static legal-static">
  <section><h1>Согласие на обработку персональных данных</h1><p>Редакция от 18.06.2026. Согласие оформляется отдельным флажком в форме записи и подтверждается нажатием кнопки «Отправить заявку».</p></section>
  <section><h2>Операторы</h2><p>ООО «Новая улыбка», ОГРН 1215800003088, ИНН 5829006081; ООО «АЭЛИТА», ОГРН 1235800004615, ИНН 5835142646.</p></section>
  <section><h2>Данные</h2><p>Имя, телефон, район, страница, дата и время заявки, источник перехода, UTM-метки и технические сведения, необходимые для защиты формы и подтверждения согласия.</p></section>
  <section><h2>Цели</h2><p>Обработка заявки, обратный звонок, подбор филиала и времени приёма, передача заявки администратору, защита от спама и подтверждение факта согласия.</p></section>
  <section><h2>Действия</h2><p>Сбор, запись, систематизация, хранение, уточнение, использование, предоставление уполномоченным работникам и обработчикам, блокирование, удаление и уничтожение с использованием средств автоматизации и без них.</p></section>
  <section><h2>Срок и отзыв</h2><p>Согласие действует до достижения целей, но не более 12 месяцев после последнего взаимодействия, если иное не требуется законом. Отзыв направляется письменным обращением по адресу соответствующего оператора. Для уточнения порядка обращения: <a href="${PHONE_LINK}">${PHONE}</a>.</p></section>
</main>`;
...
const pages = {
  '/': makePage('Новая улыбка - стоматология в Пензе', 'Стоматология Новая улыбка в Пензе: лечение зубов, имплантация, протезирование, удаление, отбеливание, врачи, акции и филиалы. Единый номер записи: +7 (967) 449-84-12.', 'Новая улыбка - современная стоматология в Пензе', ['Стоматология «Новая улыбка» в Пензе: лечение кариеса, имплантация, протезирование, виниры, удаление зубов, отбеливание и профессиональная гигиена.', `Филиалы: Светлая 11, Радужная 10 и Антонова 76. Единый номер для записи: <a href="${PHONE_LINK}">${PHONE}</a>.`], '<p><a href="/uslugi">Услуги</a> · <a href="/ceny">Цены</a> · <a href="/filialy">Филиалы</a></p>', [clinicJson(), breadcrumbJson('/', [])]),
  ...
  '/privacy': { title: 'Политика обработки персональных данных - Новая улыбка', description: 'Политика обработки персональных данных сайта стоматологии «Новая улыбка»: операторы, цели, данные, сроки и права пользователей.', html: privacyStaticHtml, jsonLd: [clinicJson(), breadcrumbJson('/privacy', [{ name: 'Политика обработки персональных данных', path: '/privacy' }])] },
  '/soglasie-na-obrabotku-personalnyh-dannyh': { title: 'Согласие на обработку персональных данных - Новая улыбка', description: 'Условия согласия на обработку имени, телефона и данных заявки при записи через сайт стоматологии «Новая улыбка».', html: consentStaticHtml, jsonLd: [clinicJson(), breadcrumbJson('/soglasie-na-obrabotku-personalnyh-dannyh', [{ name: 'Согласие на обработку персональных данных', path: '/soglasie-na-obrabotku-personalnyh-dannyh' }])] },
