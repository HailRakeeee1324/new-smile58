...
const PERSONAL_DATA_POLICY_VERSION = "18.06.2026";
const PRIMARY_OPERATOR = {
  name: "ООО «Новая улыбка»",
  ogrn: "1215800003088",
  inn: "5829006081",
  address: "440514, Пензенская область, Пензенский район, с. Засечное, ул. Светлая, д. 11, помещ. 112",
};
const SECOND_OPERATOR = {
  name: "ООО «АЭЛИТА»",
  ogrn: "1235800004615",
  inn: "5835142646",
  address: "440502, Пензенская область, Пензенский район, с. Алферьевка, Садовый проезд, д. 13",
};
...
const routeMeta = {
  home: { title: "Новая улыбка - стоматология в Пензе", description: "Стоматология Новая улыбка в Пензе: лечение зубов, имплантация, протезирование, удаление, отбеливание, акции, врачи и филиалы. Единый номер записи: +7 (967) 449-84-12.", path: "/" },
  ...
  privacy: { title: "Политика обработки персональных данных - Новая улыбка", description: "Политика обработки персональных данных сайта стоматологии «Новая улыбка»: операторы, цели, состав данных, сроки хранения, права пользователей и меры защиты.", path: routePaths.privacy },
  consent: { title: "Согласие на обработку персональных данных - Новая улыбка", description: "Условия согласия на обработку имени, телефона и данных заявки при записи через сайт стоматологии «Новая улыбка».", path: routePaths.consent },
...
function buildJsonLd(route) {
  ...
  const baseClinic = {
    ...
    name: "Новая улыбка",
    legalName: PRIMARY_OPERATOR.name,
    description: `Стоматология «Новая улыбка» в Пензе. Единый номер записи: ${PHONE}.`,
    ...
    department: [
      {
        "@type": "Dentist",
        name: "Новая улыбка — Светлая 11",
        telephone: BRANCH_PHONE_LINKS.svetlaya.replace("tel:", ""),
        address: { "@type": "PostalAddress", streetAddress: "ул. Светлая, 11", addressLocality: "Пенза", addressCountry: "RU" },
      },
      {
        "@type": "Dentist",
        name: "Новая улыбка — Радужная 10",
        telephone: BRANCH_PHONE_LINKS.raduzhnaya.replace("tel:", ""),
        address: { "@type": "PostalAddress", streetAddress: "ул. Радужная, 10", addressLocality: "Пенза", addressCountry: "RU" },
      },
      {
        "@type": "Dentist",
        name: "Новая улыбка — Антонова 76",
        telephone: BRANCH_PHONE_LINKS.antonova.replace("tel:", ""),
        address: { "@type": "PostalAddress", streetAddress: "ул. Антонова, 76", addressLocality: "Пенза", addressCountry: "RU" },
      },
    ],
...
function OperatorDetails() {
  return (
    <div className="legal-operator-grid" aria-label="Сведения об операторах персональных данных">
      {[PRIMARY_OPERATOR, SECOND_OPERATOR].map((operator) => (
        <article className="legal-operator-card" key={operator.inn}>
          <span>Оператор персональных данных</span>
          <h3>{operator.name}</h3>
          <dl>
            <div><dt>ОГРН</dt><dd>{operator.ogrn}</dd></div>
            <div><dt>ИНН</dt><dd>{operator.inn}</dd></div>
          </dl>
          <p><strong>Адрес:</strong> {operator.address}</p>
        </article>
      ))}
    </div>
  );
...
function PrivacyPage() {
  return (
    <main className="page privacy-page legal-page">
      <PageIntro
        label="Документы"
        title="Политика обработки персональных данных"
        text="Порядок обработки и защиты персональных данных пользователей сайта new-smile58.ru."
      />

      <section className="container legal-document">
        <div className="legal-document__meta">
          <span>Редакция от {PERSONAL_DATA_POLICY_VERSION}</span>
          <span>Действует для сайта new-smile58.ru</span>
        </div>
        ...
        <div className="legal-document__notice">
          ...
          <div>
            <h2>Главное</h2>
            <p>
              Через форму записи сайт запрашивает только имя, номер телефона и выбранный район. Не указывайте в форме диагнозы,
              сведения о здоровье, результаты обследований и иные медицинские данные. Они оформляются отдельно при обращении в клинику.
            </p>
          </div>
        </div>
        ...
        <section>
          ...
          <p>
            ...
            <strong> new-smile58.ru</strong>, формы записи, сервисов аналитики и защиты от автоматических обращений. Политика разработана
            в соответствии с Федеральным законом от 27.07.2006 № 152-ФЗ «О персональных данных» и применяется только к обработке,
            связанной с работой сайта и первичной обработкой обращений.
          </p>
          <p>
            Медицинская документация, сведения о состоянии здоровья и специальные категории персональных данных обрабатываются
            клиникой на иных правовых основаниях и в рамках отдельных документов, оформляемых при оказании медицинской помощи.
          </p>
        </section>

        <section>
          <h2>2. Сведения об операторах</h2>
          <p>
            Операторами персональных данных выступают юридические лица сети стоматологий «Новая улыбка» — каждое в пределах
            деятельности соответствующего филиала, договорных отношений и фактически выполняемой обработки. Координация заявок,
            поступающих с сайта, осуществляется через единый номер записи <a href={PHONE_LINK}>{PHONE}</a>.
          </p>
          <OperatorDetails />
        </section>
        ...
        <section>
          ...
          <p>Сайт может обрабатывать данные посетителей сайта и лиц, направивших заявку на запись:</p>
          <ul>
            <li>имя;</li>
            <li>номер телефона;</li>
            <li>выбранный район или филиал;</li>
            <li>адрес страницы, дата и время отправки заявки, источник перехода и UTM-метки;</li>
            <li>технические сведения: IP-адрес, тип браузера и устройства, cookies, сведения о действиях на сайте;</li>
            <li>служебные данные, подтверждающие получение согласия: версия согласия, дата и факт установки флажка.</li>
          </ul>
          <p>
            Сайт не предназначен для сбора паспортных данных, медицинских документов, диагнозов, снимков, сведений о здоровье,
            биометрических данных и данных банковских карт.
          </p>
        </section>
        ...
        <section>
          ...
          <div className="legal-table-wrap">
            <table className="legal-table">
              ...
              <tbody>
                <tr>
                  <td>Обработка заявки, обратная связь, подбор филиала и времени приёма</td>
                  <td>Имя, телефон, район, страница обращения</td>
                  <td>Согласие пользователя; действия по инициативе пользователя до заключения договора</td>
                  <td>До достижения цели, но не более 12 месяцев после последнего взаимодействия, если закон не требует иного</td>
                </tr>
                <tr>
                  <td>Защита формы от спама и обеспечение безопасности сайта</td>
                  <td>Технические сведения, токен проверки, IP-адрес</td>
                  <td>Законный интерес оператора в обеспечении безопасности и согласие в применимых случаях</td>
                  <td>В пределах срока, необходимого для проверки и расследования инцидентов, как правило не более 12 месяцев</td>
                </tr>
                <tr>
                  <td>Аналитика посещаемости и улучшение сайта</td>
                  <td>Cookies, IP-адрес, устройство, браузер, посещённые страницы, источник перехода</td>
                  <td>Согласие пользователя и настройки используемого сервиса аналитики</td>
                  <td>В пределах сроков, установленных настройками сервиса аналитики, но не дольше необходимого для заявленной цели</td>
                </tr>
                <tr>
                  <td>Подтверждение законности обработки и рассмотрение обращений</td>
                  <td>Сведения о заявке и факте согласия</td>
                  <td>Исполнение обязанностей оператора и защита законных интересов</td>
                  <td>В течение срока обработки обращения и применимых сроков исковой давности</td>
                </tr>
              </tbody>
            </table>
          </div>
          <p>
            Отправка рекламных сообщений не входит в цель формы записи. Для рекламных рассылок требуется отдельное согласие.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Операторы могут осуществлять сбор, запись, систематизацию, накопление, хранение, уточнение, извлечение,
            использование, предоставление уполномоченным работникам и обработчикам, блокирование, удаление и уничтожение данных
            с использованием средств автоматизации и без них. Обработка ограничивается целями, указанными в настоящей Политике.
          </p>
        </section>

        <section>
          <h2>6. Используемые сервисы и получатели данных</h2>
          <p>Для работы сайта могут использоваться:</p>
          <ul>
            <li>Яндекс Метрика — аналитика посещаемости;</li>
            <li>Yandex SmartCaptcha — защита формы от автоматических отправок;</li>
            <li>хостинг-провайдер и серверная инфраструктура сайта;</li>
            <li>Telegram — передача заявки в закрытую рабочую группу ответственным администраторам;</li>
            <li>уполномоченные работники операторов, которым данные необходимы для обработки обращения.</li>
          </ul>
          <p>
            Данные передаются только в объёме, необходимом для работы соответствующего сервиса и обработки заявки. При поручении
            обработки третьему лицу оператор обязан обеспечить конфиденциальность и выполнение требований законодательства.
            Трансграничная передача, если она фактически осуществляется инфраструктурой сервиса, допускается только после выполнения
        ...
        <section>
          ...
          <p>
            Сайт использует cookies и счётчик Яндекс Метрики для получения статистики, оценки источников перехода и улучшения страниц.
            Пользователь может ограничить cookies в настройках браузера; отдельные функции сайта при этом могут работать некорректно.
            Данные Метрики обрабатываются в соответствии с условиями соответствующего сервиса.
          </p>
        </section>

        <section>
          <h2>8. Хранение, защита и уничтожение данных</h2>
          <p>
            Операторы применяют необходимые правовые, организационные и технические меры: ограничение доступа, назначение ответственных,
            учёт обращений, защищённые каналы связи, контроль доступа к рабочим сервисам, резервирование и удаление данных после достижения
            цели. При достижении цели обработки, отзыве согласия либо выявлении неправомерной обработки данные блокируются и уничтожаются,
        ...
        <section>
          ...
          <ul>
            ...
            <li>потребовать уточнения, блокирования или уничтожения неполных, неточных либо незаконно обрабатываемых данных;</li>
            <li>отозвать согласие на обработку;</li>
            <li>обжаловать действия операторов в Роскомнадзоре или суде.</li>
          </ul>
          <p>
            Запрос можно направить письменным обращением по юридическому адресу соответствующего оператора. Для предварительного
            уточнения порядка обращения используется единый номер <a href={PHONE_LINK}>{PHONE}</a>. Оператор вправе запросить сведения,
            позволяющие подтвердить личность заявителя и факт взаимодействия с сайтом.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Политика действует с даты размещения на сайте. Новая редакция публикуется по тому же адресу и применяется с указанной в ней даты.
            Существенные изменения не применяются к ранее полученным данным вопреки условиям ранее выданного согласия, если иное не следует из закона.
          </p>
        </section>
      </section>
    </main>
...
function ConsentPage() {
  return (
    <main className="page privacy-page consent-page legal-page">
      <PageIntro
        label="Документы"
        title="Согласие на обработку персональных данных"
        text="Отдельное согласие для отправки заявки через форму записи на сайте new-smile58.ru."
      />

      <section className="container legal-document">
        <div className="legal-document__meta">
          <span>Редакция от {PERSONAL_DATA_POLICY_VERSION}</span>
          <span>Согласие оформляется отдельным флажком в форме</span>
        </div>

        <div className="legal-document__notice legal-document__notice--accent">
          <BadgeCheck size={28} aria-hidden="true" />
          <div>
            <h2>Как даётся согласие</h2>
            <p>
              Установка флажка «Я даю согласие…» и последующее нажатие кнопки «Отправить заявку» являются однозначным действием,
              подтверждающим согласие. Флажок изначально не установлен и должен быть выбран пользователем вручную.
            </p>
          </div>
        </div>
        ...
        <section>
          <h2>1. Операторы</h2>
          <p>
            Пользователь свободно, своей волей и в своём интересе даёт согласие следующим операторам — каждому в пределах
            деятельности соответствующего филиала и фактически выполняемой обработки:
          </p>
          <OperatorDetails />
        </section>
        ...
        <section>
          ...
          <ul>
            <li>имя;</li>
            <li>номер телефона;</li>
            <li>выбранный район: Спутник или ГПЗ;</li>
            <li>адрес страницы, дата и время отправки заявки, источник перехода и UTM-метки;</li>
            <li>технические сведения, необходимые для защиты формы и подтверждения согласия.</li>
          </ul>
          <p>
            Пользователь подтверждает, что не будет передавать через форму сведения о здоровье, диагнозы, медицинские документы,
            паспортные данные, банковские реквизиты и другие данные, не требующиеся для обратного звонка.
          </p>
        </section>
        ...
        <section>
          ...
          <ul>
            <li>регистрация и обработка заявки;</li>
            <li>обратный звонок и уточнение запроса;</li>
            <li>подбор филиала, врача и времени приёма;</li>
            <li>передача заявки ответственному администратору;</li>
            <li>защита формы от спама и подтверждение факта получения согласия;</li>
            <li>рассмотрение обращений и защита законных интересов операторов.</li>
          </ul>
          <p>Настоящее согласие не является согласием на рекламные и массовые информационные рассылки.</p>
        </section>

        <section>
          <h2>4. Действия и способы обработки</h2>
          <p>
            Разрешаются сбор, запись, систематизация, накопление, хранение, уточнение, извлечение, использование, предоставление
            уполномоченным работникам и обработчикам, блокирование, удаление и уничтожение персональных данных с использованием
            средств автоматизации и без них.
          </p>
        </section>

        <section>
          <h2>5. Лица и сервисы, участвующие в обработке</h2>
          <p>
            Для достижения целей данные могут обрабатываться уполномоченными сотрудниками операторов, хостинг-провайдером,
            сервисами Yandex SmartCaptcha и Яндекс Метрика, а также передаваться в закрытую рабочую группу Telegram для оперативной
            обработки заявки. Передача ограничивается необходимым объёмом данных.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Согласие действует с момента отправки формы до достижения целей обработки, но не более 12 месяцев после последнего
            взаимодействия по заявке, если договор, медицинское законодательство либо иное законное основание не требуют продолжения
            обработки. Сведения, подтверждающие факт согласия, могут храниться в пределах применимых сроков исковой давности.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Согласие может быть отозвано письменным обращением по юридическому адресу соответствующего оператора. Для уточнения
            порядка направления обращения можно позвонить по единому номеру <a href={PHONE_LINK}>{PHONE}</a>. В обращении следует
            указать имя, телефон, дату приблизительной отправки заявки и требование прекратить обработку. После отзыва обработка
            прекращается, кроме случаев, когда закон допускает её продолжение без согласия.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            До отправки формы пользователь должен ознакомиться с <a href={routePaths.privacy} target="_blank" rel="noreferrer">Политикой обработки персональных данных</a>
            {" "}и может проверить сведения об операторах в разделе <a href={routePaths.license} data-route-link>Лицензия и реквизиты клиники</a>.
          </p>
        </section>
      </section>
    </main>
...
function AppointmentModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const payload = {
      ...
      consentVersion: PERSONAL_DATA_POLICY_VERSION,
      consentUrl: `${window.location.origin}${routePaths.consent}`,
      privacyUrl: `${window.location.origin}${routePaths.privacy}`,
    };
    ...
    if (!payload.consentAccepted) {
      setSubmitState("error");
      setSubmitMessage("Чтобы отправить заявку, подтвердите согласие на обработку персональных данных.");
      return;
    }
        ...
        <form ref={formRef} className="appointment-form" onSubmit={handleSubmit} data-metrika-form="appointment_modal">
          ...
          <div className="appointment-form__consent-check">
            ...
            <label htmlFor="personal-data-consent">
              Я даю <a href={routePaths.consent} target="_blank" rel="noreferrer">согласие на обработку персональных данных</a>
              {" "}и подтверждаю, что ознакомился(ась) с <a href={routePaths.privacy} data-route-link>Политикой обработки персональных данных</a>.
            </label>
          </div>
    ...
    <footer className="footer">
      <div className="container footer__inner footer__inner--premium">
        ...
        <div className="footer__brand">
          ...
          <p className="footer__primary-phone">Единый номер записи: <a href={PHONE_LINK}>{PHONE}</a></p>
        </div>
      </div>
